...
namespaces=("north-default" "south-default")
numbers=("1" "2")

for from_ns in "${namespaces[@]}"; do
  for from_num in "${numbers[@]}"; do
    # Build source pod name
    from_prefix="$(echo "$from_ns" | sed 's/-default//')"
    from_pod="test-pod-${from_prefix}-${from_num}"
    ...
    for to_ns in "${namespaces[@]}"; do
      for to_num in "${numbers[@]}"; do
        ...
        to_prefix="$(echo "$to_ns" | sed 's/-default//')"
        to_svc="test-svc-${to_prefix}-${to_num}.${to_ns}.svc.cluster.local"
        ...
        expected_pass="true"
        if [[ "$from_ns" == "north-default" && "$to_ns" == "south-default" ]]; then
          expected_pass="false"
        elif [[ "$from_ns" == "south-default" && "$to_ns" == "north-default" ]]; then
          expected_pass="false"
        fi
